...
func GetMinTick(tickSpacing int) int {
	return int(math.Ceil(MinTick/float64(tickSpacing)) * float64(tickSpacing))
}

func GetMaxTick(tickSpacing int) int {
	return int(math.Floor(MaxTick/float64(tickSpacing)) * float64(tickSpacing))
}

func getMaxLiquidityPerTick(tickSpacing int) *big.Int {
	two_128 := new(big.Int).Exp(big.NewInt(2), big.NewInt(128), nil)
	two_128_minus_1 := new(big.Int).Sub(two_128, big.NewInt(1))
	denominator := int64((GetMaxTick(tickSpacing)-GetMinTick(tickSpacing))/tickSpacing + 1)
	return new(big.Int).Div(two_128_minus_1, big.NewInt(denominator))
}
...
func GetSqrtRatioAtTick(tick int) (*big.Int, error) {
	...
	// back to Q96
	if new(big.Int).Rem(ratio, Q32).Cmp(Zero) > 0 {
		return new(big.Int).Add((new(big.Int).Div(ratio, Q32)), One), nil
	} else {
